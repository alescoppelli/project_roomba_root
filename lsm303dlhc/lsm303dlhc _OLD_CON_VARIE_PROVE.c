...
// method attribute found, returns: dest[0] == <method>,    dest[1] == <self>    
    ...
    //self->writetomem =  dest[0];
       
    //mp_load_method(self->i2c, MP_QSTR_readfrom_mem, dest);   
    //self->readfrommem =  dest[0];
     
    //byte aa = ACCEL_ADDR;    
    //byte cra = CTRL_REG1_A;
    //byte s   = 0x77 ;  
    //args_write[0]=mp_obj_new_bytes((const byte*)& aa,1);
    //args_write[1]=mp_obj_new_bytes((const byte*)&cra,1);
    //args_write[2]=mp_obj_new_bytes((const byte*)&s,1);;
    //mp_load_method(self->i2c, MP_QSTR_writeto_mem, dest);  
    //mp_call_function_n_kw(dest[0],3,0,args_write);
    //mp_call_function_n_kw(writetomem,3,0,args_write);
    
    //args_write[0]=ACCEL_ADDR;
    //args_write[1]=CTRL_REG4_A;
    //args_write[2]=0x08;
    //mp_call_function_n_kw(writetomem,3,0,args_write);
    //byte aaa = ACCEL_ADDR;    
    //byte craa = CTRL_REG4_A;
    //byte ss   = 0x08 ;  
    //args_write[0]=mp_obj_new_bytes((const byte*)& aaa,1);
    //args_write[1]=mp_obj_new_bytes((const byte*)&craa,1);
    //args_write[2]=mp_obj_new_bytes((const byte*)&ss,1);;
    //mp_load_method(self->i2c, MP_QSTR_writeto_mem, dest);  
    //mp_call_function_n_kw(dest[0],3,0,args_write);
...
STATIC mp_obj_t lsm303dlhc_read_axes(mp_obj_t self_in ) {
    ...
    //mp_obj_t dest[2];	
    //mp_obj_t val;
    //mp_obj_t args_read[3];	
    accelerometer_lsm303dlhc_obj_t *self = MP_OBJ_TO_PTR(self_in);

    uint8_t data[1] = { OUT_X_L_A };
    i2c_readfrom(I2C1,ACCEL_ADDR,data, 1, true );
    ...
    //byte l = OUT_X_L_A;
    //byte n   = 1 ;  
    //args_read[0]=mp_obj_new_bytes((const byte*)& a,1);
    //args_read[1]=mp_obj_new_bytes((const byte*)&l,1);
    //args_read[2]=mp_obj_new_bytes((const byte*)&n,1);
    //val=mp_call_function_n_kw(dest[0],3,0,args_read);
    //self->accel_x.low=self->readfrommem(ACCEL_ADDR,OUT_X_L_A,1);
    //self->accel_x.low = MP_OBJ_SMALL_INT_VALUE(val);
    //byte aa = ACCEL_ADDR;    
    //byte h = OUT_X_H_A;
    //byte nn   = 1 ;  
    //args_read[0]=mp_obj_new_bytes((const byte*)& aa,1);
    //args_read[1]=mp_obj_new_bytes((const byte*)&h,1);
    //args_read[2]=mp_obj_new_bytes((const byte*)&nn,1);
    //val=mp_call_function_n_kw(dest[0],3,0,args_read);   
    //self->accel_x.high=MP_OBJ_SMALL_INT_VALUE(val);
    //self->accel_x.high=self->readfrommem(ACCEL_ADDR,OUT_X_H_A,1);

    

    //self->accel_y.low=self->readfrommem(ACCEL_ADDR,OUT_Y_L_A,1);
    //self->accel_y.high=self->readfrommem(ACCEL_ADDR,OUT_Y_H_A,1);

    //self->accel_z.low=self->readfrommem(ACCEL_ADDR,OUT_Z_L_A,1);
    //self->accel_z.high=self->readfrommem(ACCEL_ADDR,OUT_Z_H_A,1);
